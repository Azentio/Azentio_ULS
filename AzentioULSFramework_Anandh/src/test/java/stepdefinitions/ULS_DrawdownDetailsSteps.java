...
public class ULS_DrawdownDetailsSteps extends BaseClass {
	...
	public void check_when_user_add_any_drawdownbreak_up_list_view_new_row_might_be_added() throws Throwable {
		...
		String xpath = "//ion-label[text()=' " + drawdownDetailsTestdata.paymentType + " ']/parent::ion-item/ion-radio";
		for (int i = 0; i <= 50; i++) {
			try {
		...
		for (int i = 0; i <= 30; i++) {
			try {
				...
				drawdownDetailsObj.drawdownBreakUpDetailsOriginalAmountTextBox()
						.sendKeys(drawdownDetailsTestdata.OriginalAmount);
				break;
			} catch (Exception e) {
				if (i == 30) {
					Assert.fail(e.getMessage());
		...
		drawdownDetailsObj.drawdownBreakUpDetailsAmountToBePaidTextBox()
				.sendKeys(drawdownDetailsTestdata.AmountToBePaid);
		waitHelper.waitForElementToVisibleWithFluentWait(driver,
		...
		String payingCurrency = "//ion-label[text()=' " + drawdownDetailsTestdata.PayingCurrency
		...
		String paymentMode = "//ion-label[text()=' " + drawdownDetailsTestdata.Paymentmode
				+ " ']/parent::ion-item/ion-radio";
		for (int i = 0; i <= 50; i++) {
			try {
			...
			try {
				status = driver.findElement(By.xpath(
						"//ion-title[text()=' DrawDown Breakup List ']//ancestor::div[2]//parent::div//following-sibling::div//table//td[text()=' "
								+ drawdownDetailsTestdata.AmountToBePaid + " ']"))
						.isDisplayed();
				break;
			} catch (Exception e) {
				if (i == 50) {
	...
	public void verify_when_user_click_on_minus_button_that_row_will_be_deleted() throws Throwable {
		...
		driver.findElement(By.xpath(
				"//ion-title[text()=' DrawDown Breakup List ']//ancestor::div[2]//parent::div//following-sibling::div//table//td[text()=' "
						+ drawdownDetailsTestdata.AmountToBePaid + " ']//following-sibling::td[5]//ion-button"))
				.click();

		for (int i = 0; i <= 30; i++) {
			try {
				Assert.assertFalse(driver
						.findElement(By.xpath("//tr//td[text()=' " + drawdownDetailsTestdata.AmountToBePaid + " ']"))
						.isDisplayed());
				break;
