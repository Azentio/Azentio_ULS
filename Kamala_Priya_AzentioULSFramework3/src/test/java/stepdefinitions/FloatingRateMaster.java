...
public class FloatingRateMaster extends BaseClass {
	...
	FloatingRateMasterTestData floatingMasterData = jconfig.getFloatingRateMasterByName("Maker");
	...
	ExcelData excelData = new ExcelData("C:\\Users\\inindc00073\\Downloads\\UlsTestDataDesign.xlsx","FloatingRateMaster","Data Set ID");
...
@And("^Check all the fiels are avaliable in floating rate master$")
	...
	waitHelper.waitForElementToVisibleWithFluentWait(driver, floatingMstObj.floatingMaster_IndexRateTypeField(), 60, 2);
...
public void choose_the_valid_form_date() throws Throwable {
	waitHelper.waitForElementToVisibleWithFluentWait(driver, floatingMstObj.floatingMaster_ValidFormCalendar(), 60, 2);
...
Thread.sleep(4000);
	List<WebElement> calendar = driver.findElements(By.xpath("//td/span[text()='"+testData.get("CalenderDate")+"']"));
	...
	//waitHelper.waitForElementToVisibleWithFluentWait(driver, floatingMstObj.floatingMaster_CalendarToday(), 60, 2);
...
public void click_the_active_record_in_temp_view_tab() throws Throwable {
	waitHelper.waitForElementToVisibleWithFluentWait(driver, floatingMstObj.floatingMaster_TempViewactiverecord(), 60, 2);
...
public void click_the_temp_view_button_in_floating_rate_master() throws Throwable {
	waitHelper.waitForElementToVisibleWithFluentWait(driver, floatingMstObj.floatingMaster_tempviewbuttonButton(), 60, 2);
...
public void submit_the_record() throws Throwable {
	...
	waitHelper.waitForElementToVisibleWithFluentWait(driver, floatingMstObj.floatingMaster_FinalSubmitButton(), 60, 2);
...
public void verify_the_cancel_button() throws Throwable {
	...
	waitHelper.waitForElementToVisibleWithFluentWait(driver, floatingMstObj.floatingMaster_SubmitcanceButton(), 60, 2);
	floatingMstObj.floatingMaster_SubmitcanceButton().click();
	waitHelper.waitForElementToVisibleWithFluentWait(driver, floatingMstObj.floatingMaster_FinalcanceButton(), 60, 2); 
...
public void choose_the_valid_form_date_to_cancel_the_record() throws Throwable {
	waitHelper.waitForElementToVisibleWithFluentWait(driver, floatingMstObj.floatingMaster_ValidFormCalendar(), 60, 2);
	floatingMstObj.floatingMaster_ValidFormCalendar().click();
	for (int i = 0; i < 100; i++) {
		try {
			driver.findElement(By.xpath("//button[contains(text(),'"+floatingMasterData.CalenderMonth+"')]"));
			break;
		}
		catch(Exception e) {
	...
	List<WebElement> calendar = driver.findElements(By.xpath("//td/span[text()='"+floatingMasterData.CalenderDate1+"']"));
	for(WebElement element: calendar)
	{
		try
		{
...
waitHelper.waitForElementToVisibleWithFluentWaitElementToBeClickable(driver, floatingMstObj.floatingMaster_ActionIconOfactiverecord(), 60, 2);
for (int i = 0; i <10; i++) {
	try {
...
public void to_verify_the_functionality_of_search_box_with_matching_data_in_a_floating_rate_master() throws Throwable {
	for (int i = 0; i < 20; i++) {
		try {
...
public void to_verify_the_functionality_of_search_box_with_mismatch_data_in_floating_rate_master() throws Throwable {
...
public void search_the_reference_id_and_click_the_respective_action_icon_in_floating_rate_master() throws Throwable {
...
public void select_the_first_record_in_temp_view_of_floating_rate_master() throws Throwable {
	waitHelper.waitForElementToVisibleWithFluentWait(driver, floatingMstObj.floatingMaster_TempFistEditIcon(), 60, 2);
...
public void validate_the_the_invalid_detail_popup_in_floating_rate_master() throws Throwable {
	WebElement errorPopUp = driver.findElement(By.xpath("//ion-badge[contains(text(),' Maximum allowed length is 8')]"));
