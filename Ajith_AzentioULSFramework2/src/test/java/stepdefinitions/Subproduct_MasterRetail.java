...
//	}
		...
		for (int i = 0; i < 200; i++) {
			try {
				javaHelper.scrollIntoView(driver.findElement(By.xpath("//ion-label[contains(text(),'"
						+ subprdMstData.ProductCodeParameter + "')]/following-sibling::ion-radio")));
	...
	public void user_select_product_code_under_product_details_and_verify_fields() throws Throwable {
		...
		for (int i = 0; i < 200; i++) {
			try {
				javaHelper.scrollIntoView(driver.findElement(By.xpath("//ion-label[contains(text(),'"
						+ subprdMstData.ProductCode + "')]/following-sibling::ion-radio")));
				driver.findElement(By.xpath("//ion-label[contains(text(),'" + subprdMstData.ProductCode
						+ "')]/following-sibling::ion-radio")).click();
				break;
			} catch (Exception e) {
    ...
    public void user_enter_sub_product_code_under_sub_product_definition_for_verify_parameter_invalid_data() throws Throwable {
		verifyfield.verifyGivenFieldisMandatoryOrNot("Subproduct Code");
		waitHelper.waitForElementToVisibleWithFluentWait(driver, subPrdMst.Sub_Product_Subproduct_Definition(), 60, 2);
		Tab = subPrdMst.Sub_Product_Subproduct_Definition().getText();
		System.out.println("System should display The Tab as " + Tab);
		waitHelper.waitForElementToVisibleWithFluentWait(driver, subPrdMst.subProductCode(), 60, 2);
		subPrdMst.subProductCode().click();
		subPrdMst.subProductCode().sendKeys(subprdMstData.SubproductcodeParameter);
    }
	...
	public void user_enter_emi_cap_percentage_under_floating_rate_details_and_verify_fields() throws Throwable {
		...
		for (int i = 0; i < 100; i++) {
			try {
				waitHelper.waitForElementToVisibleWithFluentWait(driver, subPrdMst.Sub_Product_EMI_Cap(), 90, 2);
...
//				60, 2);
	...
	public void user_validate_the_approved_record_in_list_view() throws Throwable {
		//waitHelper.waitForElementToVisibleWithFluentWait(driver, driver.findElement(By.xpath("//span[contains(text(),'"+subprdMstData.ProductCode+"')]")), 60, 2);
		for (int i = 0; i <20; i++) {
			try {
		...
		//waitHelper.waitForElementToVisibleWithFluentWait(driver, driver.findElement(By.xpath("//span[contains(text(),'" + subprdMstData.Subproductcode + "')]")), 60, 2);
		for (int i = 0; i <20; i++) {
			try {
				String validate1 = driver.findElement(By.xpath("//span[contains(text(),'"+subprdMstData.Subproductcode+"')]")).getText();
	...
	public void user_verify_the_required_field_proper_validation_message() throws Throwable {
		String nameErrorMessage = driver.findElement(By.xpath("(//digital-form-error//ion-badge)[1]"))
				.getText();
