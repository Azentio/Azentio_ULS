...
//	JsonDataReaderWriter jsonDataReaderWriter = new JsonDataReaderWriter();
	...
	ExcelData excelData = new ExcelData("C:\\Users\\inindc00089\\Downloads\\TestDataDesignSample.xlsx","IncomeDetailsTestData", "Data Set ID");
    ...
    public void user_update_the_data_set_id_for_incentive_master1() throws Throwable {
		testData = excelData.getTestdata("AT-ID-004_D1"); 
    }
    ...
    public void user_update_the_data_set_id_for_incentive_master2() throws Throwable {
		testData = excelData.getTestdata("AT-ID-001_D1"); 
    }
    ...
    public void user_update_the_data_set_id_for_incentive_master3() throws Throwable {
		testData = excelData.getTestdata("AT-ID-005_D1"); 
    }
	@And("^user update the data set ID for Incentive Master4$")
    public void user_update_the_data_set_id_for_incentive_master4() throws Throwable {
		testData = excelData.getTestdata("AT-ID-006_D1"); 
    }
	@And("^user update the data set ID for Incentive Master5$")
    public void user_update_the_data_set_id_for_incentive_master5() throws Throwable {
		testData = excelData.getTestdata("AT-ID-007_D1"); 
    }
	    ...
	    public void search_income_details_record() throws InterruptedException  {
		 while(true)
	    	...
	    	try{  
	    		...
	    		incomeDetailsObj.IncomeDetails_SearchInputField().sendKeys(testData.get("ReferenceId"));
	    	    break;
	    	}catch(Exception e){}
	    	}
...
//     incomeDetailsObj.IncomeDetails_Next().click(); 
	...
	// incomeDetailsObj.IncomeDetails_Back().isDisplayed(); 
		...
		incomeDetailsObj.IncomeDetails_IncomeAmount().sendKeys(testData.get("Amount"));
		...
		System.out.println("TotalIncome Auto populated post entring the amount : " +TotalIncome);
   	    Assert.assertEquals(testData.get("Amount"), TotalIncome);
	    ...
	    public void validate_total_income_considered_field_auto_populated_post_entring_the_amount()  {
			...
			incomeDetailsObj.IncomeDetails_IncomeAdjustedPercent().sendKeys(testData.get("AdjustedPercent"));
			...
			System.out.println("TotalIncome Auto populated post entring the amount : " +TotalIncomeConsidered);
	   	    Assert.assertEquals(testData.get("TotalIncomeConsidered"), TotalIncomeConsidered);
	    ...
	    public void validate_total_deduction_field_auto_populated_post_entring_the_amount()  {
	    	waithelper.waitForElementToVisibleWithFluentWait(driver, incomeDetailsObj.IncomeDetails_Deduction(), 10, 1);
	 		...
	 		//incomeDetailsObj.IncomeDetails_IncomeAmount().click();
	 		incomeDetailsObj.IncomeDetails_DeductionAmount().sendKeys(testData.get("DeductionAmount"));
	    	...
	    	Assert.assertEquals(testData.get("DeductionAmount"), TotalDeduction);
	    ...
	    public void validate_total_deduction_considered_field_auto_populated_post_entring_the_amount()  {
			...
			incomeDetailsObj.IncomeDetails_DeductionAdjustedPercent().sendKeys(testData.get("DeductionAdjustedPercent"));
			...
			System.out.println("Total Deduction Considered Auto populated post entring the amount : " +TotalDeductionConsidered);
	   	    Assert.assertEquals(testData.get("DeductionAmount"), TotalDeductionConsidered);
		...
		try {
			...
			incomeDetailsObj.IncomeDetails_IncomeAmount().sendKeys(testData.get("ValidIncomeAmount"));
			incomeDetailsObj.IncomeDetails_IncomeAmount().sendKeys(Keys.ENTER);
			break;
			}
			catch(Exception e) {}
		...
		System.out.println("Modified Income Amount : " +ModifyIncomeAmount);
	   	Assert.assertEquals(testData.get("ValidIncomeAmount"), ModifyIncomeAmount);
	    ...
	    waithelper.waitForElementToVisibleWithFluentWait(driver, incomeDetailsObj.IncomeDetails_ListView_SearchIcon(), 30, 1);
	    incomeDetailsObj.IncomeDetails_SearchInputField().sendKeys(testData.get("SearchValid"));
	    incomeDetailsObj.IncomeDetails_SearchInputField().sendKeys(Keys.ENTER);
	    Thread.sleep(1000);   
	    }

	    @Then("^verify Income Details search box should not display any record wtestData.get(\"ReferenceId\")ith mismatched data$")
	    public void verify_income_details_search_box_should_not_display_any_record_with_mismatched_data() {
	    	...
	    	incomeDetailsObj.IncomeDetails_SearchInputField().sendKeys(testData.get("SearchInvalid"));
